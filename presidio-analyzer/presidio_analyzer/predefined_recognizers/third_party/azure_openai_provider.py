"""
Custom Azure OpenAI Provider for LangExtract.

This module provides a custom language model provider for Azure OpenAI,
enabling LangExtract to work with Azure-hosted OpenAI models.

Implementation based on:
- LangExtract issue #49: https://github.com/google/langextract/issues/49
- PR #38 pattern: https://github.com/google/langextract/pull/38
"""
...
# This prevents "TypeError: NoneType takes no arguments" when langextract is not installed
    ...
    class AzureOpenAILanguageModel(OpenAILanguageModel):
        ...
        def __init__(
            ...
            **kwargs
            ...
            :param api_key: Azure OpenAI API key (or set AZURE_OPENAI_API_KEY env var).
                If not provided, will automatically use managed identity (ChainedTokenCredential).
            ...
            :param azure_deployment: Explicit deployment name (optional, defaults to model_id).
            ...
            if not api_key or azure_ad_token_provider:
                ...
                logger.info(
                    ...
                    f"endpoint={self.azure_endpoint}, deployment={self.azure_deployment}"
                )
            else:
                self._client = openai.AzureOpenAI(
