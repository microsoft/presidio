...
def generate_svg(profiler_file: Path):
    ...
    try:
        print("\nGenerating SVG...")
        cmd = (
            f"gprof2dot -f pstats {profiler_file} -n 0.02 -e 0.02 "
            f"| dot -Tsvg -o {svg_file}"
        )
        subprocess.run(cmd, shell=True, check=True, capture_output=True)
        print(
            "✓ Created profiler/output/profiler.svg "
            "(red=hottest, yellow-green=hot, blue=coolest)"
        )
    except subprocess.CalledProcessError:
        print(
            "⚠️  Install: poetry add --group dev gprof2dot "
...
if __name__ == "__main__":
    ...
    subprocess.run(
        f'python -m pstats {profiler_out} <<EOF\nsort cumtime\nstats 15\nquit\nEOF',
        shell=True,
        executable='/bin/bash',
    )
    ...
    print(f"\n{'Done':-^50}")
    print("Output: profiler/output/profiler.{out,svg}")
