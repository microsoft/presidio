...
from presidio_analyzer.nlp_engine import NlpArtifacts, NLP_ENGINES
...
def test_analyze_without_entities(loaded_analyzer_engine, unit_test_guid):
    with pytest.raises(ValueError):
>>>>>>> 8d4106d... spacy required, spacy-stanza, update tests
        language = "en"
        text = " Credit card: 4095-2609-9393-4932,  my name is  John Oliver, DateTime: September 18 Domain: microsoft.com"
        entities = []
        loaded_analyzer_engine.analyze(unit_test_guid,
                                            text, entities, language,
                                            all_fields=False)
...
def test_analyze_with_empty_text(loaded_analyzer_engine, unit_test_guid):
    ...
    text = ""
    entities = ["CREDIT_CARD", "PHONE_NUMBER"]
    results = loaded_analyzer_engine.analyze(unit_test_guid,
                                                  text, entities, language,
                                                  all_fields=False)
...
||||||| parent of 8d4106d... spacy required, spacy-stanza, update tests
...
def test_added_pattern_recognizer_works(unit_test_guid):
    ...
    results = analyze_engine.analyze(unit_test_guid, text=text,
                                     entities=entities,
...
def test_when_analyze_then_apptracer_has_value(loaded_registry, unit_test_guid, nlp_engines):
    ...
    analyzer_engine_with_spacy = AnalyzerEngine(loaded_registry,
                                                app_tracer=app_tracer_mock,
                                                enable_trace_pii=True,
                                                nlp_engine=nlp_engine)
    results = analyzer_engine_with_spacy.analyze(correlation_id=unit_test_guid,
                                                 text=text,
                                                 entities=entities,
                                                 language=language,
                                                 all_fields=False,
...
def test_when_threshold_is_zero_all_results_pass(loaded_registry, unit_test_guid):
    ...
    results = analyzer_engine.analyze(unit_test_guid, text,
                                      entities, language,
                                      all_fields=False,
                                      score_threshold=0)
...
def test_when_threshold_is_more_than_half_only_credit_card_passes(loaded_registry, unit_test_guid):
    text = " Credit card: 4095-2609-9393-4932,  my phone is 425 8829090"
    language = "en"
    entities = ["CREDIT_CARD", "PHONE_NUMBER"]
...
def test_when_default_threshold_is_more_than_half_only_one_passes(loaded_registry, unit_test_guid):
    text = " Credit card: 4095-2609-9393-4932,  my phone is 425 8829090"
    language = "en"
    entities = ["CREDIT_CARD", "PHONE_NUMBER"]
...
def test_when_default_threshold_is_zero_all_results_pass(loaded_registry, unit_test_guid):
    text = " Credit card: 4095-2609-9393-4932,  my phone is 425 8829090"
    language = "en"
    entities = ["CREDIT_CARD", "PHONE_NUMBER"]
...
@pytest.mark.slow
def test_demo_text(unit_test_guid, nlp_engines):
    ...
    nlp_engine = nlp_engines["spacy_en_core_web_lg"]
    
    analyzer_engine = AnalyzerEngine(default_score_threshold=0.35, nlp_engine=nlp_engine)
    results = analyzer_engine.analyze(correlation_id=unit_test_guid, text=text, entities=None,
    ...
    assert len([entity for entity in detected_entities if entity == "DATE_TIME"]) == 1  # 1 vs 3
    ...
    assert len([entity for entity in detected_entities if entity == "PERSON"]) == 2  # 2 vs 3
    ...
    assert len(results) == 19  # 19 vs 23

def test_get_recognizers_returns_predefined(nlp_engines):
    nlp_engine = nlp_engines["spacy_en"]
    analyze_engine = AnalyzerEngine(registry=RecognizerRegistry(),
                                    nlp_engine=nlp_engine)
