...
def test_analyze_iterator_returns_list_of_recognizer_results(
    texts, expected_output, analyzer_engine_simple
):

    results = analyzer_engine_simple.analyze_iterator(list_of_texts=texts, language="en")
...
def test_analyze_dict_one_value_per_key(analyzer_engine_simple):
    ...
    results = analyzer_engine_simple.analyze_dict(input_dict=d, language="en")
...
def test_analyze_dict_on_simple_dict_with_skipping(nlp_engine, mock_registry):
    batch = {
        "PHONE_NUMBER": [
    ...
    analyzer = AnalyzerEngine(nlp_engine=nlp_engine, registry=mock_registry)
    results = analyzer.analyze_dict(
...
def test_analyze_dict_on_simple_dict(analyzer_engine_simple):
    batch = {
        "PHONE_NUMBER": [
            "Call me at 212-124-1244",
            "Phone: 5124421234",
    ...
    results = analyzer_engine_simple.analyze_dict(input_dict=batch, language="en")
...
def test_analyzer_dict_on_both_scalars_and_list(analyzer_engine_simple):
    input_dict = {
    ...
    results = list(analyzer_engine_simple.analyze_dict(input_dict, language="en"))
...
def test_analyze_dict_on_nested_dict(analyzer_engine_simple):
    nested_dict = {
    ...
    results = analyzer_engine_simple.analyze_dict(input_dict=nested_dict, language="en")
    results = list(results)
    for result in results:
...
def test_analyze_dict_nested_with_skipping(
    keys, results_length, analyzer_engine_simple
):
    nested_dict = {
        "key_a": {"key_a1": "My phone number is 212-121-1424"},
        "key_b": "www.abc.com",
    ...
    results = analyzer_engine_simple.analyze_dict(
...
def test_analyze_list_non_primitive_type_raises_error(analyzer_engine_simple):
    input_dict = {"key": ["1", "2", {"inner_key": "inner_value"}]}

    with pytest.raises(ValueError):
        list(analyzer_engine_simple.analyze_dict(input_dict=input_dict, language="en"))


def test_analyze_dict_with_unknown_object_raises_error(analyzer_engine_simple):
    class J:
        pass
