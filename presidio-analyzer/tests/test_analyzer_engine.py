...
def test_two_entities_embedded(nlp_engine):
    ...
    results = analyzer.analyze(
        text=text, all_fields=True, language="en", score_threshold=0
    )
...
def test_when_all_fields_is_true_return_all_fields(loaded_registry):
    analyze_engine = AnalyzerEngine(
    ...
    response = analyze_engine.analyze(
        text=text, all_fields=all_fields, score_threshold=threshold, language="en"
    )
...
def test_when_all_fields_is_true_all_recognizers_loaded_return_all_fields(nlp_engine):
    analyze_engine = AnalyzerEngine(
...
def test_when_allFields_is_true_and_entities_not_empty_exception():
    ...
    with pytest.raises(ValueError):
        analyze_engine.analyze(
            text=text, all_fields=True, entities=["DOMAIN_NAME"], language="en"
        )
...
def test_batch_on_simple_dict(nlp_engine):
    ...
    analyzer = AnalyzerEngine(nlp_engine=nlp_engine)
    results = analyzer.analyze_batch(batch=batch, language="en", all_fields=True)
