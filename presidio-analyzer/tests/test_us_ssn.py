...
def test_valid_us_ssn_very_weak_match():
    ...
    assert results[0].probability > 0.01 and results[0].probability < 0.31
    assert results[1].text == num2
    assert results[1].probability > 0.01 and results[0].probability < 0.31
...
def test_valid_us_ssn_weak_match():
    ...
    assert results[0].probability > 0.29 and results[0].probability < 0.41
...
def test_valid_us_ssn_very_weak_match_exact_context():
    ...
    assert len(results) == 2
    assert results[0].text == num1
    assert results[0].probability > 0.59 and results[0].probability < 0.7
    assert results[1].text == num2
    assert results[1].probability > 0.59 and results[0].probability < 0.7
...
def test_valid_us_ssn_weak_match_exact_context():
    ...
    assert len(results) == 1
    assert results[0].text == num
    assert results[0].probability > 0.5 and results[0].probability < 0.65
