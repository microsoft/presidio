...
class TestBasicLangExtractRecognizerInitialization:
    ...
    def test_when_initialized_with_mocked_ollama_then_succeeds(self, tmp_path):
        ...
        with patch(
            "presidio_analyzer.llm_utils.langextract_helper.lx", return_value=Mock()
        ):
            ...
            assert (
                len(recognizer.supported_entities) == 3
            )  # PERSON, EMAIL_ADDRESS, GENERIC_PII_ENTITY
    ...
    def test_when_model_id_missing_then_raises_value_error(self, tmp_path):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Configuration must contain 'langextract.model.model_id'",
            ):
                BasicLangExtractRecognizer(config_path=str(config_file))
    ...
    def test_when_provider_missing_then_raises_value_error(self, tmp_path):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Configuration must contain 'langextract.model.provider.name'",
            ):
                BasicLangExtractRecognizer(config_path=str(config_file))
...
class TestBasicLangExtractRecognizerAnalyze:
    ...
    def test_when_text_contains_multiple_entities_then_detects_all(
    ...
    def test_when_entities_requested_then_filters_results(self, mock_recognizer):
        ...
        with patch.object(
            mock_recognizer, "_call_llm", return_value=[person_result, email_result]
        ):
    ...
    def test_when_min_score_set_then_filters_low_confidence_results(
        self, mock_recognizer
    ):
    ...
    def test_when_langextract_raises_exception_then_exception_propagates(
        self, mock_recognizer
    ):
    ...
    def test_when_entity_has_no_mapping_and_consolidation_disabled_then_skips(
        self, tmp_path
    ):
...
class TestBasicLangExtractRecognizerParameterConfiguration:
    ...
    def test_when_config_has_params_then_overrides_defaults(self, tmp_path):
        ...
        config["langextract"]["model"]["provider"]["extract_params"][
            "max_char_buffer"
        ] = 1000
        config["langextract"]["model"]["provider"]["extract_params"][
        ...
        config["langextract"]["model"]["provider"]["language_model_params"][
            "timeout"
        ] = 120
        config["langextract"]["model"]["provider"]["language_model_params"][
            "num_ctx"
    ...
    def test_when_analyze_called_then_params_passed_to_langextract(self, tmp_path):
        ...
        with patch("langextract.extract", return_value=mock_result) as mock_extract:
            ...
            assert (
                call_kwargs["config"].provider_kwargs["model_url"]
