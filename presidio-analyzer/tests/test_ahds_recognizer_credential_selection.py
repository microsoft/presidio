...
from presidio_analyzer.predefined_recognizers.third_party.ahds_recognizer import (
...
def mock_azure_modules():
    ...
    with patch.dict(
        ...
        with patch(
            "presidio_analyzer.predefined_recognizers.third_party.ahds_recognizer.DeidentificationClient",
            mock_deid_client,
        ), patch(
            "presidio_analyzer.predefined_recognizers.third_party.ahds_recognizer.get_azure_credential",
            mock_get_credential,
        ), patch(
            "presidio_analyzer.predefined_recognizers.third_party.ahds_recognizer.PhiCategory",
            mock_phi_category,
        ):
            yield {
...
class TestAHDSRecognizerCredentialSelection:
    ...
    def test_uses_default_credential_in_development_environment(
        self, mock_azure_modules
    ):
        """Test that get_azure_credential is used when ENV=development."""
        with patch.dict(
            os.environ,
            {"ENV": "development", "AHDS_ENDPOINT": "https://test.endpoint.com"},
        ):
            mock_client_instance = MagicMock()
            mock_azure_modules["DeidentificationClient"].return_value = (
    ...
    def test_uses_chained_credential_in_production_environment(
        self, mock_azure_modules
    ):
        """Test that get_azure_credential is used when ENV=production."""
        with patch.dict(
            os.environ,
            {"ENV": "production", "AHDS_ENDPOINT": "https://test.endpoint.com"},
        ):
            mock_client_instance = MagicMock()
            mock_azure_modules["DeidentificationClient"].return_value = (
                mock_client_instance
    ...
    def test_uses_chained_credential_for_non_development_environment_values(
        self, mock_azure_modules
    ):
        ...
        for env_value in test_environments:
            with patch.dict(
                os.environ,
                {"ENV": env_value, "AHDS_ENDPOINT": "https://test.endpoint.com"},
            ):
                mock_client_instance = MagicMock()
                mock_azure_modules["DeidentificationClient"].return_value = (
                    mock_client_instance
                ...
                mock_credential = MagicMock()
                mock_azure_modules["get_azure_credential"].return_value = (
                    mock_credential
                )
                ...
                ].assert_called_once(), f"Failed for environment: {env_value}"
