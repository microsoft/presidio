...
try:
    ...
    pytest.skip(
        "Skipping test because 'langextract' is not installed", allow_module_level=True
    )
...
class TestAzureOpenAILangExtractRecognizerInitialization:
    ...
    def test_import_error_when_langextract_not_installed(self):
        ...
        with patch("presidio_analyzer.llm_utils.langextract_helper.lx", None):
            with pytest.raises(ImportError, match="LangExtract is not installed"):
                AzureOpenAILangExtractRecognizer(
                    model_id="gpt-4o", azure_endpoint="https://test.openai.azure.com/"
                )
    ...
    def test_missing_endpoint_raises_error(self, tmp_path):
        ...
        env_without_azure = {
            k: v for k, v in os.environ.items() if not k.startswith("AZURE_OPENAI")
        }
...
class TestAzureOpenAILangExtractRecognizerUsage:
    ...
    def test_environment_variables(self, mock_langextract):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            assert recognizer.model_id == "gpt-4o"
            assert (
                recognizer.azure_endpoint == "https://test-resource.openai.azure.com/"
            )
            assert recognizer.api_key == "PLACEHOLDER_NOT_A_REAL_KEY"
    ...
    def test_model_id_overrides_config(self, mock_langextract, tmp_path):
        ...
        with patch(
            "presidio_analyzer.llm_utils.config_loader.load_yaml_file"
        ) as mock_load:
...
class TestAzureOpenAILangExtractRecognizerCallMethod:
    ...
    def test_call_langextract_builds_correct_params_with_api_key(
        self, mock_langextract
    ):
        ...
        if mock_langextract.extract.called:
            ...
            assert (
                call_kwargs["language_model_params"]["api_key"]
                == "PLACEHOLDER_NOT_A_REAL_KEY"
            )
            assert (
                call_kwargs["language_model_params"]["azure_endpoint"]
                == "https://test.openai.azure.com/"
            )
            assert call_kwargs["fence_output"] is True
            assert call_kwargs["use_schema_constraints"] is False

    def test_call_langextract_without_api_key_for_managed_identity(
        self, mock_langextract
    ):
    ...
    def test_call_langextract_handles_exceptions(self, mock_langextract):
        ...
        with patch(
            "langextract.extract", side_effect=RuntimeError("Connection failed")
        ):
            ...
            with pytest.raises(RuntimeError, match="Connection failed"):
                recognizer._call_langextract(
                    ...
                    examples=[
                        MagicMock()
                    ],  # Provide mock example to avoid validation error
                )
...
class TestAzureOpenAIProvider:
    ...
    def test_provider_requires_endpoint(self):
        ...
        env_without_endpoint = {
            k: v for k, v in os.environ.items() if k != "AZURE_OPENAI_ENDPOINT"
        }
    ...
    def test_provider_managed_identity_development_env(self):
        ...
        with patch.dict(
            ...
            with patch.object(openai, "AzureOpenAI"):
                with patch(
                    "presidio_analyzer.predefined_recognizers.third_party.azure_openai_provider.get_bearer_token_provider_for_scope"
    ...
    def test_provider_import_error_without_azure_identity(self):
        ...
        with patch(
            "presidio_analyzer.predefined_recognizers.third_party.azure_openai_provider.AZURE_IDENTITY_AVAILABLE",
            False,
        ):
            with patch.object(openai, "AzureOpenAI") as mock_client:
...
class TestAzureOpenAILangExtractRecognizerParameterConfiguration:
    ...
    def test_when_config_has_params_then_overrides_defaults(
        self, mock_langextract, tmp_path
    ):
