...
import (
	...
	"github.com/Microsoft/presidio/presidio-api/cmd/presidio-api/api/anonymize-json"
...
func anonymizeJSON(c *gin.Context) {
	var anonymizeJSONAPIRequest *types.AnonymizeJsonApiRequest
	project := c.Param("project")
	if c.Bind(&anonymizeJSONAPIRequest) == nil {
		result, err := anonymizejson.AnonymizeJSON(c, api, anonymizeJSONAPIRequest, project)
		if err != nil {
			server.AbortWithError(c, http.StatusBadRequest, err)
			return
		}
...
func validateTemplate(action string, c *gin.Context) (string, error) {
	...
	case store.AnonymizeJSON:
		var anonymizeJSON types.JsonSchemaTemplate
		return bindAndConvert(anonymizeJSON, c)
	}
