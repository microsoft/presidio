...
func TestFFIValue(t *testing.T) {
	...
	// Encrypt
	result, err := FFIValue(original, locations[1], k, tw, false)
	if err != nil {
		t.Error(t, err)
	}

	result, err = FFIValue(result, locations[0], k, tw, false)
	if err != nil {
		t.Error(t, err)
	}
	expected := "this is a 608210 and 54564"

	assert.Equal(t, expected, result)

	// Descrypt
	result, err = FFIValue(expected, locations[1], k, tw, true)
	if err != nil {
		t.Error(t, err)
	}

	result, err = FFIValue(result, locations[0], k, tw, true)
	if err != nil {
		t.Error(t, err)
	}
...
func TestErrorValue(t *testing.T) {
	...
	_, err := FFIValue(original, locations[0], k, tw, false)
	assert.Error(t, err)
