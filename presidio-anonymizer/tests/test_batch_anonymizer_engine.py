...
def test_given_analyzer_result_we_anonymize_list_correctly():
    engine = BatchAnonymizerEngine()
    anonymized_results = engine.anonymize_list(texts=texts, recognizer_results_list=recognizer_results_list)
...
def test_given_complex_analyzer_result_we_anonymize_dict_correctly():
    analyzer_results = [DictAnalyzerResult(key='name',
                        ...
                        DictAnalyzerResult(key='comments',
                                           value=['called him yesterday to confirm he requested to call back in 2 days', 'accepted the offer license number AC432223', 'need to call him at phone number 212-555-5555'],
                                           recognizer_results=[[RecognizerResult('DATE_TIME', 11, 20, 0.85), RecognizerResult('DATE_TIME', 61, 67, 0.85)],
...
def test_given_custom_anonymizer_we_anonymize_dict_correctly():
    ...
    engine = BatchAnonymizerEngine()
    anonymized_results = engine.anonymize_dict(analyzer_results, operators={"DEFAULT": anonymizer_config})
    assert anonymized_results == {'name': ['<ENTITY: John>', '<ENTITY: Jill>', '<ENTITY: Jack>']}
