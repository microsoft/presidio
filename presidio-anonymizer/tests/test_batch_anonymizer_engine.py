...
from presidio_anonymizer.entities import (
    RecognizerResult, DictRecognizerResult, OperatorConfig)
...
def analyzer_results(texts, recognizer_results_list):
    return [
        DictRecognizerResult(key='name', value=texts, recognizer_results=recognizer_results_list)
    ]
...
def test_given_analyzer_result_we_anonymize_list_correctly(engine, texts, recognizer_results_list):
    ...
    anonymize_results = engine.anonymize_list(texts=new_texts, recognizer_results_list=new_recognizer_results_list)
    assert anonymize_results == ['<PERSON>', '<PERSON>', '<PERSON>', ['random', 123, True]]


def test_given_empty_recognizers_than_we_return_text_unchanged(engine, texts):
    empty_analyzer_results = [DictRecognizerResult(key='name', value=texts, recognizer_results=[])]
    anonymize_results = engine.anonymize_dict(empty_analyzer_results)
    assert anonymize_results == {'name': ['John', 'Jill', 'Jack']}


def test_given_complex_analyzer_result_we_anonymize_dict_correctly(engine, texts, recognizer_results_list):
    analyzer_results = [
...
def test_anonymize_dict_with_dict_value(engine):
    analyzer_results = [
        DictRecognizerResult(key='customer', value={'name': 'John'}, recognizer_results=[
            DictRecognizerResult(key='name', value='John', recognizer_results=[RecognizerResult('PERSON', 0, 4, 0.85)])
        ])
    ]
    anonymize_results = engine.anonymize_dict(analyzer_results)
...
def test_anonymize_dict_with_other_value(engine):
    analyzer_results = [DictRecognizerResult(key='id', value=123, recognizer_results=[])]
    anonymize_results = engine.anonymize_dict(analyzer_results)
