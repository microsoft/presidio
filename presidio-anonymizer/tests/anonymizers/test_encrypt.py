...
def test_when_anonymize_then_aes_encrypt_called_and_its_result_is_returned(
    mock_encrypt,
):
...
def test_when_verifying_an_valid_length_key_no_exceptions_raised():
    Encrypt().validate(params={"key": "128bitslengthkey"})


def test_when_verifying_an_invalid_length_key_then_ipe_raised():
    with pytest.raises(
