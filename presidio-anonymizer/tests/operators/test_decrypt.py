...
@mock.patch.object(AESCipher, "decrypt")
def test_given_anonymize_with_bytes_key_then_aes_encrypt_called_and_its_result_is_returned(
        mock_encrypt,
):
    expected_decrypted_text = "decrypted_text"
    mock_encrypt.return_value = expected_decrypted_text

    anonymized_text = Decrypt().operate(text="text", params={"key":  b'1111111111111111'})
