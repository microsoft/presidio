...
func main() {
	...
	err = storageAPI.WalkFiles(container, func(item stow.Item) {
		...
		scanResult, err = ScanAndAnalyze(&cache, item, &analyzerObj, analyzeRequest)
		if err != nil {
			log.Error(fmt.Sprintf("error scanning file: %s, error: %q", item.Name(), err.Error()))
			return
		}

		if len(scanResult) > 0 {
			err = sendResultToDataBinder(item, scanResult, cache, databinderService)
			if err != nil {
				log.Error(fmt.Sprintf("error sending file to databinder: %s, error: %q", item.Name(), err.Error()))
				return
			}
		}
