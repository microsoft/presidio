...
- Generic `IMEI` recognizer (`ImeiRecognizer`) for International Mobile Equipment Identity numbers using a formatted pattern (`##-######-######-#`), context, and Luhn checksum validation. Bare 15-digit matching is omitted to avoid collisions with other Luhn identifiers such as AMEX credit card numbers.
