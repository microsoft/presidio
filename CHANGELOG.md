...
### Analyzer
#### Added
- Added `use_memory_zone` parameter to `SpacyNlpEngine` to prevent unbounded Vocab/StringStore growth in long-running services by leveraging spaCy 3.7+ `memory_zone()` context manager. Enabled by default with graceful fallback for older spaCy versions.
