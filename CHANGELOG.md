...
### Analyzer
#### Added
- Added `use_memory_zone` parameter to `SpacyNlpEngine` to prevent unbounded Vocab/StringStore growth in long-running services by leveraging spaCy 3.7+ `memory_zone()` context manager. Opt-in (defaults to `False`) with graceful fallback for older spaCy versions.
