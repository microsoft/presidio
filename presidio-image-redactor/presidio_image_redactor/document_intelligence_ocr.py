...
class DocumentIntelligenceOCR(OCR):
    ...
    def __init__(self, endpoint = None, key = None, model_id = "prebuilt-document"):
        if model_id not in DocumentIntelligenceOCR.SUPPORTED_MODELS:
            raise ValueError("Unsupported model id: %s" % model_id)
        
        #If endpoint and/or key are not passed, attempt to get from environment variables
        if not endpoint:
        ...
        if not key or not endpoint:
            raise ValueError("Endpoint and key must be passed or set in environment variables")
    ...
    def _polygon_to_bbox(polygon):
        #Returns an tuple of left/top/width/height according to expected which covers the passed polygon
    ...
    @staticmethod
    def _page_to_bboxes(page):
        """Presidio supports tesseract format of output only, so we format in the same way"""
        ...
        #        "text": ["JOHN", "DOE"],
        #    }
        bounds = [DocumentIntelligenceOCR._polygon_to_bbox(word.polygon) for word in page.words]
    ...
    def analyze_document(self, imgbytes) -> AnalyzedDocument:
    ...
    def perform_ocr(self, image:object, **kwargs) -> dict:
        ...
        if not (len(result.pages) == 1):
            raise ValueError("DocumentIntelligenceOCR currently only supports single page documents")
