...
Persidio anonymizer comes with predefined anonymization operators but can easily be extended.
...
=== "Python"
    ...
    result = engine.anonymize(
        ...
        anonymizers_config={"PERSON": AnonymizeConfig("replace", {"new_value": "BIP"})}
    )
...
| Decrypt   | decrypt | decrypt the encrypted PII in the text using the encryption key | `key` - a cryptographic key used for the encryption is also used for the decryption. |
...
Presidio anonymizer can be easily extended to support additional anonymization operators.
See [this tutorial on adding new anonymization operators](adding_operators.md)
